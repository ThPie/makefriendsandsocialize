  ...
  const sendInvite = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke('send-referral-invite', {
        body: { inviter_id: user.id, recipient_email: inviteEmail, personal_message: personalMessage || undefined },
      });
      if (error) throw error;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-border bg-card">
        <CardHeader>
          <CardTitle className="font-display text-xl flex items-center gap-2">
            <Gift className="h-5 w-5 text-[hsl(var(--accent-gold))]" />
            Your Referral Code
          </CardTitle>
          <CardDescription>Share your unique code with friends and earn rewards</CardDescription>
        </CardHeader>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="border-border bg-card">
          <CardContent className="pt-6 text-center">
            <Mail className="h-8 w-8 mx-auto mb-2 text-[hsl(var(--accent-gold))]" />
        ...
        <Card className="border-border bg-card">
          <CardContent className="pt-6 text-center">
            <Trophy className="h-8 w-8 mx-auto mb-2 text-[hsl(var(--accent-gold))]" />
      ...
      <Card className="border-border bg-card">
        <CardHeader>
          <CardTitle className="font-display text-xl flex items-center gap-2">
            <Trophy className="h-5 w-5 text-[hsl(var(--accent-gold))]" />
            Reward Progress
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            <div><Badge variant="secondary" className="text-sm">{rewardTier.tier}</Badge></div>
            {rewardTier.next && (
          ...
          <div className="space-y-3">
            ...
            <div className={`flex items-center justify-between p-3 rounded-lg ${totalReferrals >= 10 ? 'bg-[hsl(var(--accent-gold))]/10 border border-[hsl(var(--accent-gold))]/20' : 'bg-card border border-border'}`}>
              <div className="flex items-center gap-3">
                <span className="text-lg">👑</span>
                <div>
      ...
      <Card className="border-border bg-card">
        <CardHeader>
          ...
          <CardDescription>Send a personalized invitation email to a friend</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Input type="email" placeholder="friend@example.com" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} />
          <Textarea placeholder="Add a personal message (optional)" value={personalMessage} onChange={(e) => setPersonalMessage(e.target.value)} rows={3} />
          <Button onClick={sendInvite} disabled={!inviteEmail || isSending} className="w-full">
          ...
          <CardContent>
            <div className="space-y-3">
              {referrals.slice(0, 5).map((referral) => (
                <div key={referral.id} className="flex items-center justify-between p-3 bg-muted border border-border rounded-lg">
                  <div>
                    <p className="font-medium text-sm">{referral.referred_email || 'Anonymous'}</p>
                    <p className="text-xs text-muted-foreground">{new Date(referral.created_at).toLocaleDateString()}</p>
                  </div>
                  <Badge variant={referral.status === 'converted' ? 'default' : 'secondary'} className="capitalize">
